...
public class StockApiClient {
    ...
    public void patch(String id, PatternRepresentation patternRepresentation) {
        ...
        UriComponents uriBuilder =
                UriComponentsBuilder.fromUri(stockServiceUrl)
                        .path(path)
                        .build();
        this.webClient.patch().uri(uriBuilder.toUri()).body(Mono.just(patternRepresentation), PatternRepresentation.class).exchangeToMono(
                x -> {
