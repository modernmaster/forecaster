...
import uk.co.jamesmcguigan.forecaster.facade.GoogleSheetRepresentation;
...
public class StockRetrievalService extends HttpServlet implements StockRetrieval {
    ...
    private static final String SENDING_LIVE_PRICES_TO_TRENDS_MESSAGE_QUEUE = "Sending {} live-prices to uk.co.jamesmcguigan.forecaster.service.trends message queue";
    ...
    private static final String RECEIVED_REQUEST_TO_UPDATE_ALL_STOCK = "Received request to update all stock {}";
    private static final String RECEIVED_REQUEST_TO_UPDATE_ALL_STOCKS = "Received request to update all stocks";
    ...
    public void updateStocks() {
        log.info(RECEIVED_REQUEST_TO_UPDATE_ALL_STOCKS);
    ...
    public void updateStock(String id) {
        log.info(RECEIVED_REQUEST_TO_UPDATE_ALL_STOCK, id);
        List<Stock> stocks = getStocksFromResource();
        stocks.stream()
    ...
    private void processStock(Stock stock) {
        ...
        stockApiClient.patch(stock);
        LivePriceEvent livePriceEvent = livePriceTransformer.transform(stock);
        log.debug(SAVING_LIVE_PRICES, livePriceEvent.getSymbol());
        livePriceRepository.save(livePriceEvent);
        log.debug(SENDING_LIVE_PRICES_TO_TRENDS_MESSAGE_QUEUE, livePriceEvent.getSymbol());
        trendsMessageProducer.sendMessage(livePriceEvent);
    }
